...
util.inherits(MultiplexerServer, EventEmitter);

MultiplexerServer.prototype._onmessage = function(message) {
	if(message == null) {
		return;
	}
	try {
...
MultiplexerServer.prototype._ondisconnect = function() {
	var streams = this._streams;
	for(var i in streams) {
		streams[i]._ondisconnect()
	}
}
MultiplexerServer.prototype._send = function(name, data) {
	this._ws.send(PACK([name, data]));
}

MultiplexerServer.prototype.getStream = function(name) {
	var stream = this._streams[name];
	if(stream == null) {
...
Stream.prototype.send = function(data) {
	this._controller._send(name, data);
}

module.exports.MultiplexerServer = MultiplexerServer;